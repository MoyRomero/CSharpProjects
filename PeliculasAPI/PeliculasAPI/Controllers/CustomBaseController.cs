...
namespace PeliculasAPI.Controllers
{
    public class CustomBaseController:ControllerBase
    {
        ...
        protected async Task<List<TDTO>> Get<TEntidad, TDTO>(PaginacionDTO paginacionDTO, IQueryable<TEntidad> queryable)
            where TEntidad : class
        {
            await HttpContext.InsertarParametrosPaginacionEnCabecera(queryable, paginacionDTO.RecordsPorPagina, paginacionDTO.Pagina);
        ...
        protected async Task<ActionResult> Patch<TEntidad,TDTO>(int id, JsonPatchDocument<TDTO> patchDocument)
            ...
            where TEntidad: class, IId
        {
            if (patchDocument == null) return BadRequest("No se han agregado valores para reemplazar en la entidad.");
